...
Согласно техническому заданию предполагалось использование облачной LLM. Однако в проекте была выбрана интеграция через `spring-ai-starter-model-ollama`, предназначенная для работы с локальным сервером **Ollama**. 
Переход на облачную модель потребовал бы замены используемого Spring AI Starter и перенастройки слоя интеграции с LLM. При этом Ollama предоставляет OpenAI-совместимый API, поэтому логика взаимодействия с моделью остается аналогичной и при необходимости может быть адаптирована для работы с облачным сервисом.
---
